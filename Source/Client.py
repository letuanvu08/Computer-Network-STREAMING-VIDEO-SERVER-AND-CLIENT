				...
				if data:
					...
					if currFrameNbr > self.frameNbr:# Discard the late packet
						if currFrameNbr -self.frameNbr > 1:
							self.numberFrameLoss = currFrameNbr -self.frameNbr-1
						self.frameNbr = currFrameNbr
						self.updateMovie(self.writeFrame(rtpPacket.getPayload()))
					if self.numberFrameRec!=0:
						print(f"RTP packet loss rate:{self.numberFrameLoss/(self.numberFrameLoss + self.numberFrameRec)}")
			except:
			...
			if self.sessionId == session:
				if int(lines[0].split(' ')[1]) == 200:
					if self.requestSent == self.SETUP:
						...
						x=threading.Thread(target=self.listenRtp)
